...
public class RagedEnemy : MonoBehaviour
{
    ...
    {
        cooldownTimer = 0;
        fireballPrefab[0].transform.position = firePoint.position;
        // fireballPrefab[0].GetComponent<EnemyProjectile>.ActivateProjectile();
    }    

    // private void FindFireball()
    // {
    //     for(int i = 0; i < fireballPrefab.Length; i++)
    //     {
    //         if(!fireballPrefab[i].activeInHierarchy)
    //         {
    //             return i;
    //         }
    //     }
